...
def generate_security_report(target_type, target_value):
    ...
    security_report = SecurityReport.objects.create(target_type=target_type, target_value=target_value)
    # Create ScanResult entries
    for data_source, data in report_data.items():
        ScanResult.objects.create(security_report=security_report, data_source=data_source, report_data=data)
    return security_report


def analyze_malicious_item(malicious_item):
    # Fetch data from security data sources
    detection_result = fetch_detection_result(malicious_item.item_type, malicious_item.value)
